---
home: true
icon: home
title: 首页◾武门扉
heroImage: /assets/coding.gif
heroImageDark: /assets/home.gif
# layout: BlogHome
# bgImage: /assets/coding.gif
# heroText: "" # 主标题
# tagline: "" # 副标题
# heroImage: /assets/home.gif # logo heroImageDark可设置夜间模式
# heroFullScreen: true

actions:
  - text: 待办事项
    link: /todo/
    icon: signs-post
    type: primary

  - text: 漫游指南
    icon: lightbulb
    link: /guide/

# features:
#   - title: Markdown 增强
#     icon: fab fa-markdown
#     details: 新增文字对齐、上下角标、脚注、标记、任务列表、数学公式、流程图、图表与幻灯片支持
#     link: /zh/guide/markdown/

#   - title: 幻灯片页面
#     icon: person-chalkboard
#     details: 添加幻灯片页面以显示你喜欢的内容
#     link: /zh/guide/layout/slides

#   - title: 布局增强
#     icon: fab fa-bluetooth
#     details: 添加路径导航、页脚、改进的导航栏、改进的页面导航等。
#     link: /zh/guide/layout/

footer: 路漫漫其修远兮，吾将上下而求索.
---

## Learning Site

- [stack overflow](https://stackoverflow.com/)
- [MDN - Mozilla](https://developer.mozilla.org/zh-CN/)
- [现代 JavaScript 教程](https://zh.javascript.info/)
- [30 seconds of code](https://www.30secondsofcode.org/)
- [Js Tips](https://www.jstips.co/)
- [w3schools](https://www.w3schools.com/)
- [Devdocs](https://devdocs.io/javascript/)
- [HTML5 Tricks](https://www.html5tricks.com/)

## Third-Party Library

- [RxJS - Reactive Extensions Library for JavaScript](https://rxjs.dev/)
- [Mock.js - 生成随机数据，拦截 Ajax 请求](http://mockjs.com/)
- [Fuse.js - Powerful, lightweight fuzzy-search library, with zero dependencies](https://www.fusejs.io/)
- [Print.js - A tiny javascript library to help printing from the web](https://printjs.crabbly.com/)
- [DPlayer - video player](https://dplayer.diygod.dev/)
- [Swiper - 开源、免费、强大的触摸滑动插件](https://www.swiper.com.cn/)

## CSS Library

- [Tailwind - CSS Preprocessor](https://tailwindcss.com/)
- [Stylus - CSS Preprocessor](https://stylus-lang.com/)
- [Less - CSS Preprocessor](https://lesscss.org/)
- [Sass - CSS Preprocessor](https://sass-lang.com/)
- [PostCSS - 一个用 JavaScript 工具和插件转换 CSS 代码的工具](https://www.postcss.com.cn/)
- [Clippy - CSS clip-path maker](https://bennettfeely.com/clippy/)
- [Styled Components](https://styled-components.com/)
- [CSS-Tricks](https://css-tricks.com/guides/)

## Animation Library

- [Splitting.js](https://splitting.js.org/)
- [Animate - Just-add-water CSS animations](https://animate.style/)
- [Animista](https://animista.net/)
- [GSAP](https://gsap.com/)

## React Library

- [ahooks](https://ahooks.js.org/zh-CN)
- [overreacted - React Author Blog](https://overreacted.io/)
- [Ant Motion - Animation specification and components of Ant Design.](https://motion.ant.design/index-cn)

## Blog Site

- [简书 - IT 技术 > 前端](https://www.jianshu.com/techareas/frontend)
- [掘金 - 前端 > 最新](https://juejin.cn/frontend?sort=newest)
- [博客园 - 分类 > 前端开发](https://www.cnblogs.com/cate/108703/)
- [CSDN - 前端 > 全部](https://blog.csdn.net/nav/web)
- [SegmentFault - 思否](https://segmentfault.com/)

#### personal

- [网道 - 一个文档网站，提供互联网开发文档](https://wangdoc.com/)
- [ES6 入门教程 - 阮一峰](https://es6.ruanyifeng.com/)
- [张鑫旭](https://www.zhangxinxu.com/)
- [冴羽的博客](https://github.com/mqyqingfeng/Blog)
- [木易杨前端进阶](https://muyiy.cn/)
- [校招面试题](https://github.com/mqyqingfeng/frontend-interview-question-and-answer)

## Reading Corner

- [OSCHINA - 开源中国](https://www.oschina.net/)
- [Web 开发路线图](https://github.com/ccloli/developer-roadmap-zh-CN)
- [现代 Web 开发魔法全书](https://github.com/dexteryy/spellbook-of-modern-webdev)
- [成为 Web 全栈工程师](https://github.com/bmorelli25/Become-A-Full-Stack-Web-Developer)
- [Awesome 列表系列](https://github.com/sindresorhus/awesome)
- [30 seconds of code](https://www.30secondsofcode.org)

## Tools

- [在线制作 icon 图标](https://www.bitbug.net/)
- [Carbon - 创建并分享源代码的精美图像](https://carbon.now.sh/)
- [BootCND - 静态资源分发](https://www.bootcdn.cn/)
- [Squoosh - 图片压缩](https://squoosh.app/)
- [Tinypng - 图片压缩](https://tinypng.com/)
- [Drawio - 流程图](https://www.drawio.com/)
- [菜鸟工具](https://www.jyshare.com/)
- [在线工具 - 可安装的应用](https://tool.lu/)
- [API 设计、开发、测试一体化协作平台](https://apifox.com/)

## Query

- [印记中文 - 各细分点整理](https://docschina.org/)
- [正则表达式](https://ihateregex.io/)

#### Materials/Models

- [17 素材网](https://www.17sucai.com/)
- [Wallhaven 壁纸](https://wallhaven.cc/)
- [unDrow 插画](https://undraw.co/illustrations)

#### Development Language Index

- [Can I Use](https://caniuse.com/)
- [PYPL PopularitY of Programming Language](https://pypl.github.io/PYPL.html)
- [TIOBE](https://www.tiobe.com/tiobe-index/)

## 特别说明

本站的属于个人博客网站，不存在任何商业用途。本站使用[VuePress](https://github.com/vuejs/vuepress) 搭建，主题配置[vuepress-theme-hope](https://github.com/vuepress-theme-hope/vuepress-theme-hope)，特别鸣谢。本站部分图片等资源均源自互联网，若存在侵权请联系本人删除。
